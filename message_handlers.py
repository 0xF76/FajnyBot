...
async def selectStudent(self, message):
	...
	choise = random.choice(list(notChosen.items()))
	chosenFile = open('chosen.txt', 'a')

	if (len(self.chosenIDs) + 1 < len(self.data)):
		self.chosenIDs.append(choise[0])
		chosenFile.write("{} ".format(choise[0]))
	else:
		self.chosenIDs = []
		chosenFile.seek(0)
		chosenFile.truncate()
	...
	chosenFile.close()
	print('{} - {}'.format(choise[0], choise[1]))
	await message.channel.send(embed = discord.Embed(title=choise[1], color=0x1ac8eb))
